...
class ADeleteUser(SuccessMessageMixin, DeleteView):
	model = models.User
	template_name = 'admin/confirm_delete3.html'
...
def create_user(request):
	...
	if request.method == 'POST':
		...
		if userType == "Publisher":
			a = models.User(first_name=first_name, last_name=last_name, username=username, email=email, password=password,
					 is_publisher=True)
			a.save()
...
class ACreateChat(LoginRequiredMixin, CreateView):
	form_class = ChatForm
	model = models.Chat
	template_name = 'admin/chat_form.html'
...
class AManageBook(LoginRequiredMixin, ListView):
	model = models.Book
	template_name = 'admin/manage_books.html'
	context_object_name = 'books'
	paginate_by = 3
...
class ADeleteBook(LoginRequiredMixin, DeleteView):
	model = models.Book
	template_name = 'admin/confirm_delete2.html'
...
class ADeleteRequest(LoginRequiredMixin, ListView):
	...
	template_name = 'admin/delete_request.html'
	context_object_name = 'feedbacks'
	paginate_by = 3

	def get_queryset(self):
		return models.DeleteRequest.objects.order_by('-id')
...
class AFeedback(LoginRequiredMixin, ListView):
	...
	def get_queryset(self):
		return models.Feedback.objects.order_by('-id')
...
def asearch(request):
	...
	if (len(data) == 0):
		...
		if files:
			return render(request, 'admin/result.html', {'files': files, 'word': word})
		return render(request, 'admin/result.html', {'files': files, 'word': word})
...
def send_feedback(request):
	if request.method == 'POST':
		...
		a = models.Feedback(feedback=feedback)
		a.save()
